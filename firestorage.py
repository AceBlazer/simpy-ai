...
def uploadRetrievedImagesCore(customer_name, project_name):
    ...
    for imgName in os.listdir(path):
        ...
        if os.path.isfile(imgName):
            blob = storage.bucket(os.environ['FIRESTORAGE_BUCKET']).blob(
                imgName)  # intended name of file in Firebase Storage
            ...
            for img in os.listdir(imgName):
                img = imgName+"/"+img
                blob = storage.bucket(os.environ['FIRESTORAGE_BUCKET']).blob(
                    img)  # intended name of file in Firebase Storage
                blob.upload_from_filename(img)  # path to file on local disk


def uploadRetrievedImages(customer_name, project_name):
    try:
        p = Process(target=uploadRetrievedImagesCore,
                    args=(customer_name, project_name))
        p.start()
...
def uploadIndexes(customer_name, project_name):
    ...
    for indexFile in os.listdir(path):
        ...
        blob = storage.bucket(os.environ['FIRESTORAGE_BUCKET']).blob(
            indexFile)  # intended name of file in Firebase Storage
